...
TEST_CASE("disjoint_components", "[mtetcol]")
{
    ...
    std::vector<Index> cycle_indices = {0, 3, 6, 10, 14, 18};

    mtetcol::DisjointComponents components(9, cycles, cycle_indices);
    ...
    SECTION("Second comp")
    {
        ...
        components.register_cycle(mtetcol::signed_index(4, false));

        components.extract_components(polyhedra, polyhedron_indices);

        size_t num_polyhedra = polyhedron_indices.size() - 1;
        REQUIRE(num_polyhedra == 1);
        REQUIRE(polyhedron_indices[1] == 2);
    }
